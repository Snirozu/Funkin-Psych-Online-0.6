...
class GameBanana {
	public static function searchMods(?search:String, page:Int, response:(mods:Array<GBSub>, err:Dynamic) -> Void) {
		Thread.create(() -> {
			var http = new Http('https://gamebanana.com/apiv11/Game/8694/Subfeed?_nPage=${page}&_sSort=default&_csvModelInclusions=Mod'
				+ (search != null ? '&_sName=$search' : ''));
	...
	public static function getMod(id:String, response:(mod:GBMod, err:Dynamic) -> Void) {
		Thread.create(() -> {
			var http = new Http("https://api.gamebanana.com/Core/Item/Data?itemtype=Mod&itemid="
				+ id
				+

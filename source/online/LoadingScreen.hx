...
class LoadingScreen extends Sprite {
	...
	override function __enterFrame(delta) {
		...
		roseCirc.graphics.beginFill(0xFCFCFC);
		roseCirc.graphics.drawCircle(Lib.application.window.width / 2 + thetaSin * Math.cos(thetaAngle(0)),
			Lib.application.window.height / 2 + thetaSin * Math.sin(thetaAngle(0)), 5);
		roseCirc.graphics.endFill();
		// coolShader.update(delta);

		roseCirc1.graphics.clear();
		roseCirc1.graphics.beginFill(0xFCFCFC);
		roseCirc1.graphics.drawCircle(Lib.application.window.width / 2 + thetaCos * Math.sin(thetaAngle(2)),
			Lib.application.window.height / 2 + thetaCos * Math.cos(thetaAngle(2)), 5);
		roseCirc1.graphics.endFill();

		roseCirc2.graphics.clear();
		roseCirc2.graphics.beginFill(0xFCFCFC);
		roseCirc2.graphics.drawCircle(Lib.application.window.width / 2 + thetaSin2 * Math.sin(thetaAngle(1)),
			Lib.application.window.height / 2 + thetaSin2 * Math.cos(thetaAngle(1)), 5);
		roseCirc2.graphics.endFill();

		roseCirc3.graphics.clear();
		roseCirc3.graphics.beginFill(0xFCFCFC);
		roseCirc3.graphics.drawCircle(Lib.application.window.width / 2 + thetaCos2 * Math.cos(thetaAngle(3)),
			Lib.application.window.height / 2 + thetaCos2 * Math.sin(thetaAngle(3)), 5);
		roseCirc3.graphics.endFill();
	}
}
// i give on trails

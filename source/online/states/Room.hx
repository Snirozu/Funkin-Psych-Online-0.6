...
class Room extends MusicBeatState {
				...
				switch (curSelected) {
					...
					case 2:
						...
						if (!selfPlayer.hasSong
							&& GameClient.room.state.song != ""
							&& (Mods.getModDirectories().contains(GameClient.room.state.modDir) || GameClient.room.state.modDir == "")) {
	...
	function updateTexts() {
		...
		player1Text.text = GameClient.room.state.player1.name
			+ "\n"
			+ "Ping: "
			+ GameClient.room.state.player1.ping
			+ "ms"
			+ "\n\n"
			+ "Last Song Summary\n"
			+ "Score: "
			+ GameClient.room.state.player1.score
			+ "\n"
			+ "Accuracy: "
			+ GameClient.getPlayerAccuracyPercent(GameClient.room.state.player1)
			+ "%\n"
			+ "Sicks: "
			+ GameClient.room.state.player1.sicks
			+ "\n"
			+ "Goods: "
			+ GameClient.room.state.player1.goods
			+ "\n"
			+ "Bads: "
			+ GameClient.room.state.player1.bads
			+ "\n"
			+ "Shits: "
			+ GameClient.room.state.player1.shits
			+ "\n"
			+ "Misses: "
			+ GameClient.room.state.player1.misses
			+ "\n";
		...
		if (GameClient.room.state.player2 != null && GameClient.room.state.player2.name != "") {
			...
			p2.alpha = 1;
			player2Text.text = GameClient.room.state.player2.name
				+ "\n"
				+ "Ping: "
				+ GameClient.room.state.player2.ping
				+ "ms"
				+ "\n\n"
				+ "Last Song Summary\n"
				+ "Score: "
				+ GameClient.room.state.player2.score
				+ "\n"
				+ "Accuracy: "
				+ GameClient.getPlayerAccuracyPercent(GameClient.room.state.player2)
				+ "%\n"
				+ "Sicks: "
				+ GameClient.room.state.player2.sicks
				+ "\n"
				+ "Goods: "
				+ GameClient.room.state.player2.goods
				+ "\n"
				+ "Bads: "
				+ GameClient.room.state.player2.bads
				+ "\n"
				+ "Shits: "
				+ GameClient.room.state.player2.shits
				+ "\n"
				+ "Misses: "
				+ GameClient.room.state.player2.misses
				+ "\n";

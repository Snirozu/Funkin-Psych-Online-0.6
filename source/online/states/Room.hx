...
class Room extends MusicBeatState {
	...
	public function new() {
		...
		chatIcon.antialiasing = ClientPrefs.globalAntialiasing;
		...
		playIcon.antialiasing = ClientPrefs.globalAntialiasing;
		...
		GameClient.room.onMessage("gameStarted", function(message) {
			Waiter.put(() -> {
				...
				Paths.currentModDirectory = GameClient.room.state.modDir;
		...
		GameClient.room.onMessage("checkChart", function(message) {
			Waiter.put(() -> {
				try {
					...
					if (Paths.getModDirectories().contains(GameClient.room.state.modDir) || GameClient.room.state.modDir == "") {
						Paths.currentModDirectory = GameClient.room.state.modDir;
						try {
			...
			if (controls.ACCEPT || FlxG.mouse.justPressed) {
				switch (curSelected) {
					...
					case 2:
						...
						if (!selfPlayer.hasSong && GameClient.room.state.song != "" && (Paths.getModDirectories().contains(GameClient.room.state.modDir) || GameClient.room.state.modDir == "")) {
							Paths.currentModDirectory = GameClient.room.state.modDir;
							try {
								GameClient.send("verifyChart", Md5.encode(Song.loadRawSong(GameClient.room.state.song, GameClient.room.state.folder)));
							}

...
class RequestState extends MusicBeatSubstate {
	...
	public static function requestDownload(url:String, ?prompt:String = "Do you want to download this file", ?onDownloadFinished:String->Void,
			?disableTrusting:Bool = false, ?yesCallback:Void->Void) {
		request(prompt, url, nowTrusting -> {
			if (nowTrusting != null) {
				var splitURL = url.split("//");
				if (nowTrusting)
	...
	public static function request(prompt:String, url:String, yesCallback:(nowTrusting:Null<Bool>) -> Void, noCallback:Void->Void,
			?disableTrusting:Bool = false) {
